...
if r == "About Project":
    ...
    Question =[{"Question": "Find top 10 highest revenue generating products", "query": '''select product_id, cast(sum(sale_price * quantity)as decimal(10,2)) as total_revenue 
    from result2 
    group by product_id
    order by total_revenue desc
    limit 10 '''},
                {"Question":"Find the top 5 cities with the highest profit margins","query":'''select city,
    cast(sum((sale_price - cost_price)*quantity)/sum(sale_price * quantity) as decimal(10,4)) as profit_margin 
    from result2
    group by city
    order by profit_margin desc
    limit 5'''},
                {"Question":"Calculate the total discount given for each category","query":'''select category,
    cast(sum(discount * sale_price* quantity)as decimal(15,2)) as total_discount 
    from result2
    group by category
    order by total_discount desc'''},
                {"Question":"Find the average sale price per product category","query":'''select sub_category,
    cast(avg(sale_price)as decimal(10,2)) as average_sale_price
    from result2
    group by sub_category
    order by average_sale_price desc'''},
                {"Question":"Find the region with the highest average sale price","query":'''select region,
    cast(avg(sale_price)as decimal(10,2)) as average_sale_price
    from result2
    group by region
    order by average_sale_price desc
    limit 1'''},
                {"Question":"Find the total profit per category","query":'''select category,
    cast(sum(profit * quantity)as decimal (10,2)) as total_profit
    from result2
    group by category
    order by total_profit desc'''},
                {"Question":"Identify the top 3 segments with the highest quantity of orders","query":'''select segment,
    sum(quantity) as total_quantity
    from  result2
    group by segment 
    order by total_quantity desc
    limit 3'''},
                {"Question":"Determine the average discount percentage given per region","query":'''select region,
    avg(discount_percent) as average_discount_percent
    from result2
    group by region
    order by average_discount_percent desc'''},
                {"Question":"Find the product category with the highest total profit","query":'''select sub_category,
    cast(sum(profit *quantity)as decimal(15,2)) as total_profit
    from result2
    group by sub_category
    order by total_profit desc
    limit 1'''},
                {"Question":"Calculate the total revenue generated per year","query":'''select extract(year from order_date) as order_year,
...
if r=='Self written':
    ...
    Question =[{"Question": "Get average discount percent for each category", "query": '''select o.category,
avg(p.discount_percent) as average_disc_percent
from  orders o 
join products p on o.order_id = p.order_id
group by o.category '''},
                {"Question":"Get total sales for each region ","query":'''select o.region,
cast(sum(p.sale_price * p.quantity)as decimal(15,2)) as total_sales
from orders o 
join products p on o.order_id = p.order_id
group by region'''},
                {"Question":"Find top 10 orders with highest profit margin ","query":'''select o.order_id,
...
inner join products p 
on o.order_id = p.order_id'''},
                {"Question":"Retrieve total profit for each order with product details","query":'''select o.order_id,p.product_id,p.sub_category,
sum(p.profit) as total_profit
from orders o 
inner join products p 
on o.order_id = p.order_id
group by o.order_id,p.product_id,p.sub_category'''},
                {"Question":"Retrieve all orders for south region","query":'''select o.order_id,o.order_date,p.product_id,p.sub_category
from orders o 
inner join products p 
on o.order_id = p.order_id
where o.region = 'South';'''},
                {"Question":"Select top 5 products by profit","query":'''select p.product_id,p.sub_category,
sum(p.profit) as total_profit
from orders o 
inner join products p 
on o.order_id = p.order_id
...
order by total_profit desc 
limit 5;'''},
                {"Question":"Retrieve total sales for each city from highest to lowest","query":'''select o.city,
cast(sum(p.sale_price * p.quantity)as decimal(15,2)) as total_sales 
from orders o 
inner join products p 
on o.order_id = p.order_id
...
order by total_sales desc;'''},
                {"Question":"Get the complete list of products and its details delivered to Florida","query":'''select p.* from orders o 
join products p 
on o.order_id = p.order_id
